...
class Document(Base):
    ...
    # engine's get_document tool, was retired with the loop (D11).
...
class Chunk(Base):
    ...
    # Full-text index for Level 2's keyword search. Generated by Postgres from `text`, so
    # ingest never writes it and it can never drift from the chunk it indexes.
...
class Run(Base):
    """One turn on the support graph: the step trail a person reads.
    ...
    at this scale queries steps across runs. Resuming a run is the LangGraph checkpointer's
    job (thread_id = run id); this row is the audit trail and the conversation-reload source.
    ...
    # Unused since D10: stage outputs live in the LangGraph checkpoint. Kept rather than
    # dropped so rows written by the retired pipeline still load.
