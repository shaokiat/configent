...
- [x] **G2.6** `hi`, `thanks`, `what can you do?` and a bare follow-up on the previous answer
      each produce a reply and **zero** tickets in the mock service (D9), asserted in a test
      and scored as `converse` cases on the golden set. A real technical question phrased
      casually routes to `answer` or `ticket`, **never** `converse` — `converse` is the one
      path that skips scoring, so a platform fact stated there is ungrounded by
      construction.
      *Verified 2026-09-09: `tests/test_demo_inputs.py` (13 inputs, incl. empty, emoji-only,
      a 400-line paste and an injection attempt) and `tests/test_ticket_confirmation.py`.*
...
| **W2-2a** ✅ | `app/agent/pipeline.py`, `app/routers/clients.py`, `apps/web/.../ChatPanel.tsx` | **Nothing is filed without confirmation.** Stream the draft as a proposal; `POST /c/{id}/runs/{run_id}/ticket` files it on confirm, reusing `stage_ticket` | D9, **and the correctness half** — with no auto-filing, a greeting files zero tickets whether or not triage exists, so this alone satisfies the brief's queue-precision criterion. `stage_ticket` is already idempotent per run and stage_seq (D4), so the endpoint is thin. If only one of 2a/2b ships, it is this one. |
| **W2-2b** ✅ | `app/agent/pipeline.py`, `prompts/gcp-platform-support/ticket_draft.md` | **Triage.** `_TICKET_SCHEMA` gains `route: "ticket"\|"converse"` and `reply`; `stage_ticket` runs only on `route == "ticket"`; `PipelineResult.escalated` becomes `outcome: "answer"\|"converse"\|"ticket"` | D9, and **before W2-4** — the golden set has to encode the outcome this produces. The quality half: without it `hey` gets a ticket proposal it does not deserve, wasting a draft call. Rides inside the escalate-draft call that already runs, so no new stage and no cost on the answerable path. **`converse` replies are the one user-facing text produced with no passages in front of the model** — constrain the prompt to acknowledge-and-redirect and forbid stating any platform fact (G2.6). |
...
`W2-1` → `W2-3` → `W2-2` → `W2-2a` → `W2-2b` → `W2-4` → **`W2-5` (commit)** → `W2-6` →
`W2-7` → `W2-9` → `W2-8` → `W2-10` → `W2-11`

`W2-2a` first: it stops the bleeding on its own. `W2-2b` before the golden set, because the
set has to encode the three-way outcome it introduces. Neither is droppable into week 4 now
— the demo bug is the reason week 2 has this shape.
